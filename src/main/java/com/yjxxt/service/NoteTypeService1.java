...
public class NoteTypeService1 {
    ...
    private Boolean checkNoteTypeNameUnique(String typeName, Integer userId) {
        long count = noteTypeMap.values().stream()
                .filter(n -> n.getTypeName().equals(typeName))
                .filter(n -> !n.getUserId().equals(userId)).count();
        if (count==1) {
            return true;
        }
        return false;
